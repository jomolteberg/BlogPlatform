...
export default function CreatePost() {
  const [formData, setFormData] = useState<FormData>({ title: '', description: '', image: null });
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!formData.title || !formData.description || !formData.image) {
      alert('Please fill out all fields and attach an image.');
      return;
    }
  
    const apiEndpoint = 'https://localhost:5001/BlogPost'; // Replace with your actual API endpoint
  
    // Prepare FormData for submission
    const submissionData = new FormData();
    submissionData.append('Title', formData.title); // Ensure this matches the DTO field name
    submissionData.append('Content', formData.description); // Ensure this matches the DTO field name
      ...
      <form onSubmit={handleSubmit} className="relative">
        <div className="overflow-hidden rounded-lg border border-gray-300 shadow-sm focus-within:border-orange-500 focus-within:ring-1 focus-within:ring-orange-500">
          ...
          <textarea
            ...
            onChange={handleInputChange}
            className="block w-full resize-none border-0 p-2.5 text-gray-900 placeholder-gray-400 focus:ring-0 sm:text-sm sm:leading-6"
  ...
  <div className="flex items-center space-x-3">
    <span className="text-sm italic">{fileName || "No file selected"}</span>
    <button
      type="button"
      className="inline-flex items-center rounded-full px-3 py-2 text-left text-gray-400 hover:text-gray-500"
      onClick={() => document.getElementById('image-upload')?.click()}
    >
      <PaperClipIcon className="-ml-1 mr-2 h-5 w-5" aria-hidden="true" />
